...
This document outlines the design and implementation plan for merging the Prefill-Decode Load Balancer (PDLB) functionality into the existing SGLang Router. The goal is to create a unified routing solution that supports both traditional load balancing and prefill-decode disaggregated routing.
